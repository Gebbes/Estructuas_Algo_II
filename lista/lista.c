...
bool lista_iter_avanzar(lista_iter_t *iter){
    if(lista_iter_al_final(iter)) return false; // Si el actual es NULL (ultimo) devuelvo false
...
void *lista_iter_ver_actual(const lista_iter_t *iter){
    if(lista_iter_al_final(iter)) return NULL;       //Si estoy en el ultimo, devuelvo NULL
    return iter->actual->dato;
}


bool lista_iter_al_final(const lista_iter_t *iter){
    return (!(iter->actual));     //If el actual es el NULL
}
...
bool lista_iter_insertar(lista_iter_t *iter, void *dato){
    if(iter->actual == iter->lista->primero){
        bool op = lista_insertar_primero(iter->lista,dato);
        if(!op)return false;
    ...
    if(lista_iter_al_final(iter)){
        bool op = lista_insertar_ultimo(iter->lista,dato);
        if(!op)return false;
        iter->anterior = iter->actual;
...
void *lista_iter_borrar(lista_iter_t *iter){
    ...
    if(lista_iter_ver_actual(iter) == lista_ver_primero(iter->lista)){
        iter->actual = iter->actual->siguiente;
    ...
    if(lista_iter_ver_actual(iter) == lista_ver_ultimo(iter->lista))ultimo = true;
    ...
    if(!ultimo)iter->lista->ultimo = aux_nodo;
    if(ultimo)iter->lista->ultimo = iter->anterior;
